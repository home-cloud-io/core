...
// UpdateApps (if enabled) will check for and install app updates every day at 3am
func (u *updater) AutoUpdateApps() {
	c := cron.New()
	c.AddFunc("0 3 * * *", u.updateApps)
	go c.Start()

}

func (u *updater) updateApps() {
	ctx := context.Background()
	u.logger.Info("")
	settings := &v1.DeviceSettings{}
	err := kvclient.Get(ctx, kvclient.DEFAULT_DEVICE_SETTINGS_KEY, settings)
	...
	if !settings.AutoUpdateApps {
		u.logger.Info("auto update apps not enabled")
		return
	}

	storeApps, err := u.controller.GetAppsInStore(ctx)
	if err != nil {
		u.logger.WithError(err).Error("failed to get apps in store")
		return
	}

	installedApps, err := u.k8sclient.InstalledApps(ctx)
	if err != nil {
		u.logger.WithError(err).Error("failed to get installed apps")
		return
	}
		...
		for _, store := range storeApps {
			if installed.Name == store.Name {
				if semver.Compare(store.Version, installed.Spec.Version) == 1 {
					err := u.controller.UpdateApp(ctx, u.logger, &v1.UpdateAppRequest{
...
func (u *updater) CheckForContainerUpdates(ctx context.Context, logger chassis.Logger) ([]*v1.ImageVersion, error) {
	...
	images, err := u.k8sclient.CurrentContainerVersions(ctx)
	if err != nil {
